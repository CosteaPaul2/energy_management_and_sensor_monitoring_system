...
import { DeviceDetails, DeviceWithId } from "../interfaces/DeviceWithId";
...
const DeviceTable: React.FC<DeviceTableProps> = () => {
  ...
  const fetchDeviceUserId = async (username: String) => {
    try {
      ...
      const response = await axios.get(
        `${DEVICE_URL}/device/user/username/${username}`,
        config,
      );
  ...
  const fetchUsers = async () => {
    try {
      ...
      setUsers(
        response.data.map((user: { id: string; username: string }) => ({
          value: user.id,
          label: user.username,
        })),
      );
  ...
  const handleCreateDevice = async (newDevice: Omit<DeviceDetails, "userId">) => {
    try {
      const token = Cookies.get("jwtToken");
      const config = { headers: { Authorization: `Bearer ${token}` } };
      ...
      const createdDevice: DeviceWithId = response.data;
  ...
  const openAssignModal = (deviceId: string) => {
    ...
    const handleViewDetails = (deviceId: string) => {
        navigate("/device-details", { state: { deviceId } });
    };
  ...
  const rows = devices.map((device) => (
    <Table.Tr key={device.id}>
      ...
      <Table.Td>
      <Button onClick={() => handleViewDetails(device.id)} variant="subtle">
          {device.id}
        </Button>
      </Table.Td>
      <Table.Td>
  ...
  return (
    <>
        ...
        <Table verticalSpacing="sm">
          <Table.Thead>
            <Table.Tr>
              ...
              <Table.Th>Device Description</Table.Th>
              <Table.Th>Device Hourly</Table.Th>
      ...
      {user?.roles?.includes("ADMIN") && (
        <>
          <Group mt="md">
            <Button onClick={() => setCreateModalOpen(true)}>Create Device</Button>
          </Group>
...
export const unassignDevice = async (deviceId: string) => {
  ...
  try {
    await axios.post(`${DEVICE_URL}/device/unassign/${deviceId}`, {}, config);
    console.log(`Device ${deviceId} unassigned successfully`);
  } catch (error) {
    console.error("Error unassigning device:", error);
  }
};
